...
// Déclaration interface Post
interface Post {
  _id?: string
  _rev?: string // Gestions des révisions
  post_name: string
  post_content: string
  attributes: {
...
export default {
  ...
  mounted() {
    this.initDatabase() // Initialisation de la bdd à la montée du composant
  },
  ...
  methods: {
    ...
    updatePost() {
      this.storage
        ...
        .catch((error) => {
          console.error('Erreur lors de la modification du doc:', error)
        })
    },
    deletePost() {
      if (this.post?._id) {
        this.storage
          ...
          .catch((error) => {
            console.error('Erreur lors de la suppression du doc:', error)
          })
      }
    }
  }
